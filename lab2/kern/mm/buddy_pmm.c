...
free_area_t buddy_free_area[11]; // 支持最大2^10页的伙伴系统
...
static struct Page *buddy_alloc_pages(size_t n)
{
    ...
    for (int current_order = order; current_order <= MAX_ORDER; current_order++)
    {
        ...
        {
            list_entry_t *le = list_next(&buddy_free_area[current_order].free_list);
...
static void basic_check(void)
{
    ...
    {
        free_list_store[order] = buddy_free_area[order].free_list;
        nr_free_store[order] = buddy_free_area[order].nr_free;
        list_init(&buddy_free_area[order].free_list);
        buddy_free_area[order].nr_free = 0;
    }
    ...
    for (int order = 0; order <= MAX_ORDER; order++)
    {
        total_free += buddy_free_area[order].nr_free;
    }
    assert(total_free == 3);
